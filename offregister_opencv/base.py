...
def dl_install_opencv(branch='master', tag='4.1.1', extra_cmake_args=''):
    ...
    if run("grep '^[[:blank:]]*[^[:blank:]#;]' /etc/apt/sources.list | grep -qF '-security main'", warn_only=True).failed:
        ...
        sudo('apt update')
    apt_depends('build-essential', 'checkinstall', 'cmake', 'pkg-config', 'yasm', 'git', 'gfortran',
                'libjpeg8-dev', 'libpng-dev', 'software-properties-common', 'libtiff-dev',
                ...
                'libtbb-dev', 'qt5-default', 'libatlas-base-dev', 'libfaac-dev', 'libmp3lame-dev',
                'libtheora-dev', 'libvorbis-dev', 'libxvidcore-dev', 'libopencore-amrnb-dev',
                'libopencore-amrwb-dev', 'libavresample-dev', 'x264', 'v4l-utils', 'libprotobuf-dev',
                'protobuf-compiler', 'libgoogle-glog-dev', 'libgflags-dev', 'libgphoto2-dev',
                'libeigen3-dev', 'libhdf5-dev', 'doxygen', 'python3-dev', 'python3-pip', 'python3-numpy',
                'python3-testresources', 'libeigen3-dev',
                # Hmm:
                'libopencv-dev', 'opencv-data'
                )
    run("mkdir -p '$HOME/repos'", shell_escape=False)
    with cd('$HOME/repos'):
        clone_or_update(repo='opencv', team='opencv', branch=branch, tag=tag, skip_reset=True)
